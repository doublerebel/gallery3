(function ($) {
  ...
  $.fn.gallery_context_menu = function() {
    var $this = $(this), $contextMenu = $this.find('.g-context-menu');
    if ($contextMenu.find('li').length) {
      var $menuItems = $contextMenu.hover(
        function() {
          $(this).not('in_progress').find("ul").slideDown("fast", function() { $(this).addClass('in_progress'); })
        },
        function() {
          $(this).find("ul").slideUp("slow", function() { $(this).removeClass('in_progress'); });
        }
      ).find("*").removeAttr('title').find("ul").hide();
      $menuItems.find(".g-dialog-link").gallery_dialog();
      $menuItems.find('.ui-icon-rotate-cw, .ui-icon-rotate-ccw').click( function() { $.gallery.AjaxRotate(this); return false; });
      $menuItems.find('.ui-icon-star').click( function() { $.gallery.AjaxSetAlbumCover(this); return false; });
    }
    return $this;
  };
